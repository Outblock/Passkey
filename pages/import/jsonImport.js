...
const JsonImport = ({onOpen}) => {
  ...
  const handleImport = async (e) => {
    try {
      ...
      const result = await jsonToKey(keystore, password)
      const pubK = Buffer.from(result.getPublicKeySecp256k1(false).data())
      const pubKeyHex = pubK.toString('hex').replace(/^04/, "")
      const address = await findAddressWithKey(pubKeyHex)
      setLoading(false)
      if (!address) {
        onOpen()
      }
